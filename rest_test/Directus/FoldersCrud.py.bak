...
class DirectusFoldersCrud:
    ...
    def test_02_authenticate_login_to_retrieve_temporary_access_token_ai(self, api_client):
        """
        Combined test executing 4 API operations
        """
        ...
        with allure.step("Step 1: POST /auth/login"):
            logger.info(f"🚀 Step 1: {'POST'} {step1_endpoint}")
            step1_payload = {'email': 'admin@example.com', 'password': 'admin123'}  # Updated credentials
            response1 = api_client.post(
                endpoint=step1_endpoint,
        ...
        with allure.step("Step 2: POST /folders"):
            ...
            step2_payload = {'name': f'Folder_{uuid.uuid4()}', 'parent': 0}  # Use random string for folder name
            ...
            # Extract fields from response for use in subsequent steps
            try:
                extracted_id = response2.json_data["data"]["id"]  # Retrieve ID from response
            except (KeyError, TypeError, IndexError):
        ...
        with allure.step("Step 3: GET /folders"):
            ...
            # Validate that the response has a record with the retrieved ID and folder name
            folder_found = any(folder['id'] == extracted_id and folder['name'] == step2_payload['name'] for folder in response3.json_data['data'])
            assert folder_found, f"Folder with ID {extracted_id} and name {step2_payload['name']} not found in Step 3 response"


        # Step 4: Delete the folder
        allure.dynamic.parameter("Step_4_Sl_No", 23)
        allure.dynamic.parameter("Step_4_Method", "DELETE")
        allure.dynamic.parameter("Step_4_Endpoint", "/folders")

        step4_endpoint = "/folders"

        with allure.step("Step 4: DELETE /folders"):
            logger.info(f"🚀 Step 4: {'DELETE'} {step4_endpoint}")
            response4 = api_client.delete(
                endpoint=step4_endpoint,
                json_payload={'id': extracted_id}  # Use the same ID retrieved above in delete request
            )
