"""
Generated Test File
Generated on: 2026-08-22 15:11:09
...
class TestGeneratedAPIs:
    ...
    def test_01_obtain_the_openid_access_token_ai(self, api_client):
        ...
        # Create payload for obtaining access token
        payload = {
            "client_id": "synaptix-portal",
            "username": "admin.synaptix",
            "grant_type": "password",
            "password": "admin123"
        }
        ...
        with allure.step("Step 1: POST /realms/synaptix-technologies/protocol/openid-connect/token"):
            ...
            # Extract access token for use in subsequent steps
        ...
        with allure.step("Step 2: GET /admin/realms/synaptix-technologies/groups"):
            ...
            response2 = api_client.get(
                ...
                headers={"Authorization": f"Bearer {bearerToken}"}  # Using the bearer token for authorization
            )
            ...
            assert expected_group_names.issubset(actual_group_names), f"Expected group names {expected_group_names} not found in the response"

    def test_02_obtain_the_openid_access_token_ai(self, api_client):
        """
        Combined test executing 2 API operations
        """
        ...
        with allure.step("Step 2: GET /admin/realms/synaptix-technologies/groups"):
            ...
            # Validate group names in the response
            expected_group_names = {"Employees", "IT", "Finance", "HR", "Services"}
            if response2.json_data and isinstance(response2.json_data, list):
                ...
                assert expected_group_names.issubset(actual_group_names), f"Expected groups {expected_group_names} not found in response"
                logger.info(f"📌 Group names validated successfully: {actual_group_names}")
            else:
                logger.error("⚠️  Invalid response format or no groups found")
                pytest.fail("Invalid response format or no groups found")
